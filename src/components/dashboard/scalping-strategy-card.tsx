// src/components/dashboard/scalping-strategy-card.tsx
"use client";
...
export function ScalpingStrategyCard({ latest, market, history, openPosition }: StrategyCardProps) {
  const config = STRATEGY_CONFIG.scalping;
  const isRelevantPosition = openPosition?.strategy === 'scalping';

  if (!latest || !market || history.length < 2) {
    return (
      <Card>
        <CardHeader><CardTitle>Diagnóstico Scalping</CardTitle></CardHeader>
  ...
  return (
    <Card className={clsx("border-2", 
      ...
      <CardHeader>
        ...
        <CardDescription>Requiere {config.minBuyConditions} condición de compra. Vende por TP, SL o RSI alto.</CardDescription>
      </CardHeader>
      <CardContent>
        {isRelevantPosition ? (
          <>
